DROP FUNCTION IF EXISTS public.webpush_config_for_service();

CREATE OR REPLACE FUNCTION public.webpush_config_for_service()
RETURNS TABLE (public_key text, private_key text, subject text, trigger_secret text)
LANGUAGE sql
SECURITY DEFINER
SET search_path = private, public
AS $$
  SELECT c.public_key, c.private_key, c.subject, c.trigger_secret
